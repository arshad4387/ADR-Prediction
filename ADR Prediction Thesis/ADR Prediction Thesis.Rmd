...
Furthermore, we expand the baseline dataset by approximately 25% relative to the original study by compiling and reconciling the most recent data releases from the SIDER and Drug-Gene Interaction Database (DGIdb), establishing a comprehensive study cohort of 973 unique drug compounds. In addition, an interactive application was developed that allows users to query a given drug name and retrieve its associated predicted adverse effects, demonstrating the practical applicability of the proposed framework in a real world, user facing context.

In the remainder of this paper, Section 2 describes the datasets, Section 3 methods, followed by results and discussions in sections 4 and 5, respectively.
...
```{r dataset-dimensions, echo=FALSE}
dataset_dims <- data.frame(
  `Feature Type` = c("Drug-Gene Interaction", "Chemical Fingerprints", "Side Effect Matrix"),
  `Dimensionality` = c("973 × 3,647", "973 × 1,024", "973 × 5,779"),
  check.names = FALSE
)

knitr::kable(
  dataset_dims,
  caption = "Summary of dataset dimensions.",
  align = "lc"
)
```
...
The drug side effect matrix is highly sparse, with a positive rate well below 10% for most side effects. In the entire dataset, about 97% of the data represents the negative class and remaining as positive class.
...
Exploratory data analysis (EDA) was performed to characterize the primary structural and distribution properties of the adverse drug reaction (ADR) dataset. Figure 1 and 2 provides an overview of the data landscape, partitioned by individual drug burdens and clinical symptom prevalence. Specifically, Figure 1 outlines the cohort of drugs associated with the highest cumulative number of distinct side effects. A substantial proportion of these top tier drugs including Pregabalin, Aripiprazole, and Citalopram predominantly target the central nervous system (CNS), a therapeutic class inherently prone to systemic and broad spectrum adverse reactions due to complex neuromodulatory pathways.

```{r fig-most-common-drug-a, echo=FALSE, fig.cap="Identification of drugs associated with the greatest volume of unique side effects.\\label{fig:fig-most-common-drug-a}", out.width="95%", fig.align="center"}

knitr::include_graphics("figures/1a.png")
```

Complementing this, Figure 2 illustrates the overall frequency of specific clinical symptoms across the entire drug library, measured by the absolute number of unique compounds that trigger them. The distribution reveals a highly concentrated subset of traditional physiological distress markers such as nausea, headache, and vomiting accounts for a disproportionate majority of all recorded events in the database. Taken together, these dual distributions highlight the severe class imbalance and density variations inherent to post market ADR profiles, establishing a critical baseline justification for the robust mathematical matrix factorization techniques implemented in this study.

```{r fig-most-common-drug-b, echo=FALSE, fig.cap="Frequency ranking of the most pervasive clinical symptoms observed across the entire compound library.\\label{fig:fig-most-common-drug-b}", out.width="95%", fig.align="center"}

knitr::include_graphics("figures/1b.png")
```

Figure 3 illustrates the distribution of the number of side effects associated with each drug. Several drugs are associated with more than 200 side effects, while a small subset is associated with as many as 800, further underscoring the heterogeneity of side-effect burden across drugs.
```{r fig-se-distribution, echo=FALSE, fig.cap="Distribution of side effects", out.width="95%", fig.align="center"}

knitr::include_graphics("figures/distribution_of_se.png")
```
Given that the majority of the proposed models rely on similarity between drug profiles, Figure 4 presents a heatmap illustrating drug similarity based on shared side-effect profiles. Distinct clustering patterns are observed, suggesting that subsets of drugs share similar adverse reaction profiles. The heatmap further demonstrates the sparsity of the underlying interaction matrix, with positive drug–side effect associations representing only a small fraction of all possible combinations. Collectively, these visualisations highlight the heterogeneous nature of ADR profiles and the pronounced class imbalance characteristic of the prediction task.

```{r fig-se-heatmap, echo=FALSE, fig.cap="Drug Similarity Heatmap", out.width="100%", fig.align="center"}
...
kable(
  ...
  col.names = c("Method", "Mean AUROC", "Mean AUPR")
)
```
...
```{r fig-roc, echo=FALSE, fig.cap="ROC Curve for all methods", out.width="90%", fig.align="center"}
...
kable(auroc_full, caption = "AUROC scores across five folds with mean ± standard deviation",
      col.names = c("Method", "F1", "F2", "F3", "F4", "F5", "Mean SD")
      )
```
...
To further translate these findings into a practically usable form, an interactive web application was developed to communicate model predictions beyond the scope of static evaluation metrics. This application enables end users to input a drug name and receive a ranked list of predicted adverse drug reactions generated by the best-performing model. Figures 7 and 8 present screenshots of this application, illustrating its interface and the structure of its output. The application can be run locally; the corresponding source code, datasets, and deployment instructions are provided in the paper's accompanying repository. [GitHub repository](https://github.com/arshad4387/ADR-Prediction)

```{r fig-app, echo=FALSE, fig.cap="Interactive Application For ADR Prediction", out.width="90%", fig.align="center"}
