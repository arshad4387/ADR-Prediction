...
Figure 5 presents the sensitivity and specificity of each evaluated method across decision thresholds ranging from 0.1 to 0.9. The heatmaps illustrate each model's ability to correctly identify true ADR associations (sensitivity) while correctly rejecting negative associations (specificity).
...
Figure 6 presents the Receiver Operating Characteristic (ROC) curves for all evaluated methods. VKR NMF achieved the strongest overall discrimination between positive and negative ADR associations, producing the highest AUROC among all evaluated approaches. VKR SVD and the Naïve Baseline also demonstrated competitive ROC performance, whereas SVM Linear exhibited the weakest discrimination.
...
```
To further translate these results and communicate the findings in a more practical and accessible form, an application has been developed to demonstrate the predictive capability of the best-performing model. Figures 7 and 8 present snapshots of this interactive web application, which allows users to input a drug and obtain a ranked list of predicted adverse drug reactions. The app can be run locally where the code, datasets, and deployment steps are all available in the paper’s. [GitHub repository](https://github.com/arshad4387/ADR-Prediction)
